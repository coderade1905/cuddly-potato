    ...
    $("#sms-btn").click(
        function () {
            ...
            $.ajax({
                ...
                method : "POST",
                dataType : "JSON",
                data : {FN : $("input[name=FN]").val(), phone : $("input[name=PhoneNumber]").val(), Pass : $("input[name=Password]").val(), CPass : $("input[name=C-Password]").val(), Pass : $("input[name=Password]").val(), Gender : $("input[name='gender']:checked").val(), Class : $("select[name=cla] option:selected").text(),otp : otp, fields : fields, values : values, id : school_id, type : "t"},
                success: function(response)
                {
                    if (response.status == 200)
                    {
